...
Built for high-reliability environments, AgentOne integrates **LangGraph stateful multi-agent graphs**, **Hybrid RAG** (Dense Embeddings + Okapi BM25 with Reciprocal Rank Fusion), **bidirectional PII masking**, **adversarial prompt injection defense**, **Human-In-The-Loop (HITL) checkpointing**, and an automated **LLM-as-a-Judge Evaluation Suite**.
...
- **Supervisor Node**: Coordinates stateful sub-task dispatching, loop bounding, and top-level routing.
- **Triage & Intent Agent**: Classifies incoming requests, assesses priority (`P1_CRITICAL` through `P4_LOW`), evaluates customer sentiment, and extracts entity identifiers.
- **RAG & Self-Reflection Agent**: Retrieves relevant operational runbooks and evaluates document sufficiency (*Self-RAG* loop) to avoid ungrounded answers.
- **Action & Tool Dispatcher**: Executes external actions (ticket creation, refund disbursement, account profiling) with automatic risk classification.
- **Critic & Compliance Agent**: Evaluates synthesis for factual groundedness against retrieved runbooks, strips generic AI boilerplate, and formats actionable resolutions.

### 2. Advanced Hybrid RAG Engine
- **Dense Vector Search**: Computes high-dimensional semantic embeddings with cosine similarity and metadata filtering.
...
- **Semantic Chunking**: Automatically structures Markdown documents along heading hierarchies (`#`, `##`, `###`) and paragraph boundaries into coherent knowledge units.
...
- **Bidirectional PII Redaction**: Reversibly masks emails, phone numbers, credit card numbers, SSNs, and API keys with structured placeholders (`[EMAIL_1]`, `[CARD_1]`).
- **Human-in-the-Loop (HITL) Checkpoints**: Automatically intercepts high-risk operations (e.g. refunds $\ge \$500$, data deletion, schema alterations) and holds execution until operator sign-off via REST or UI.
...
- **Context Precision & Recall**: Evaluates retrieval ranking quality and knowledge coverage.
- **Synthetic Benchmark Suite**: Pre-configured benchmark test harness runnable programmatically or via UI.
...
- **Multi-Provider LLM Engine**: Seamlessly switch between Google Gemini, OpenAI GPT-4o, Anthropic Claude 3.5, or the built-in deterministic Mock Provider for zero-key test execution.
...
4. **Configure Environment Variables (Optional):**
   ...
   cp .env.example .env
   ```
   *(Note: The system defaults to `DEFAULT_LLM_PROVIDER=mock`, allowing you to run all agents, tests, RAG, and UI out-of-the-box without API keys. Provide `GEMINI_API_KEY`, `OPENAI_API_KEY`, or `ANTHROPIC_API_KEY` in `.env` to connect live foundation models).*
...
Running the automated benchmark harness (`POST /api/eval/run`) on the standardized enterprise dataset yields:
...
| `GET` | `/api/rag/search` | Query hybrid RAG index with configurable $\alpha$ balance |
...
├── .agent/
│   └── rules/
│       └── autonomous_build.md       # Agent build standards & guidelines
├── .github/
│   └── workflows/
...
│   │   ├── state.py                  # TypedDict AgentState & domain models
...
│   │   ├── tool_executor.py          # Action dispatcher & system APIs
